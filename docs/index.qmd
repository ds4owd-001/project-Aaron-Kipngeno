...
Data was analysed using Rtool software for descrptive statistics such frequency, percentage, mean and standrard deviations. Findings was presented using tables, and charts. Inferential statistisc was computed using chi–square test , to examine associations of social and economic factors , and willingness to invest in sanitations.

Ehical considitations was sought in this study.
...
library(ggplot2)
# plot1- bar graph
bar_graph <- ggplot(data = sanitation_clean, aes(x = age_bracket, fill = age_bracket)) +
...
#plot- box plot
# Create a box plot
box_plot <- ggplot(sanitation_clean, aes(x = education, y = household_size)) +
...
# Create a gt table
 table1 |>
  as_gt() |>
  tab_header(
    title = " Table 1.0: Demographic information of the respondents"
  ) |>
  tab_style(
    style = list(
      cell_text(size = "12px"),
...
#Create a histogram for the household_size variable
fig_1 <- ggplot(sanitation_clean, aes(x = household_size)) +
...
Table_2 shows, majority of respondents 76% were willing to investigate. The study investigate the level of willingness, 29% were not willing, 20% very much willing and 48% were willing to invest, with significant associations at p-Value (0.001) as shown in Table_3

```{r}
Table_2 <- sanitation_clean |> 
  tbl_summary(include = c(willingness_to_invest))
  
Table_3 <- sanitation_clean |> 
  tbl_summary(include = c(willingness_to_invest)) |>
  as_gt() |>
...
Stdudy investiagted associations of gender,age, religion, knowledge on satiationn, presence of latrine, benefit of use of toilet, sanitation laws and willingness to invest in sanitation
...
#genderand willinness to invest
...
the study investigated influence of employement status, household income , sanitation organizations, knowldge on finacial institutions, and beng member to saving and lending groups.

#chi-square test statistics.

```{r}
#ingluence of employement status
