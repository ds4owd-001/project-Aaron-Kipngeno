...
#bar graphs showing distribution of respondents by age, brackets
```{r}
...
ggplot(sanitation_clean, aes(x = household_size)) +
  ...
  labs(title = "Household Size Distribution",
       ...
       y = "Count")
```
# willlinges to invest in sanitation
```{r}
sanitation_clean |>
  tbl_summary(include = c(willingness_to_invest)) 
  
sanitation_clean |>
  tbl_summary(include = c(level_of_willingness))

sanitation_clean |>
  tbl_cross(row = level_of_willingness,
            col = willingness_to_invest,
            percent = "cell") |>
  add_p() # update the column header
